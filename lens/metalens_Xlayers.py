...
num_layers = 2
scriptName = "metalens_Xlayers2_img"
...
Sy = 2 * pml_size + design_region_height * num_layers + 2 # + 5
...
source_center = [0, -(design_region_height / 2 + 0.75), 0] # normally 1.5 instead of 0.75 # Source 1.5 µm below lens
...
for i in range(num_layers):
    design_regions.append(mpa.DesignRegion(
        design_variables,
        volume=mp.Volume(
            center=mp.Vector3(y = (i - 0.5 * num_layers + 0.5) * design_region_height),
...
NearRegions = [ # region from which fields at focus point will be calculated
    mp.Near2FarRegion(
        center=mp.Vector3(0, num_layers * design_region_height / 2 + 0.75), # normally 1.5 instead of 0.75 # 1.5 µm above lens
