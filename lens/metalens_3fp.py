...
parser.add_argument("--f") # Script name
parser.add_argument("--v", type=int, default=0) # Verbose
parser.add_argument("--p", action="store_true") # Periodic
parser.add_argument("--r", action="store_true") # Random start
args = parser.parse_args()

scriptName = args.f
symmetry = False # Impose symmetry around x = 0 line
mp.verbosity(args.v) # amount of info printed during simulation
#mp.divide_parallel_processes(6)
...
design_region_width = 9 # width of layer
design_region_height = [0.5, 0.445, 0.35, 0.216] # height of layer
...
# System size 
Sx = design_region_width + 2 * empty_space if args.p else 2 * pml_size + design_region_width + 2 * empty_space 
...
# Boundary conditions
pml_layers = [mp.PML(pml_size, direction=mp.Y)] if args.p else [mp.PML(pml_size)]
# pml_layers = 

# Source
fwidth = 0.03 # 0.2
source_center = [0, -(half_total_height + 0.4), 0] # Source 0.4 um below lens
...
geometry = [
    mp.Block(center=design_region.center, size=design_region.size, material=design_region.design_parameters)
    for design_region in design_regions
    ]

"""
# Add SiO2 block after lens
block_height = 8
geometry = [
    mp.Block(center=design_regions[0].center, size=design_regions[0].size, material=design_regions[0].design_parameters),
    mp.Block(center=design_regions[1].center, size=design_regions[1].size, material=design_regions[1].design_parameters),
    mp.Block(center=design_regions[2].center, size=design_regions[2].size, material=design_regions[2].design_parameters),
    mp.Block(center=design_regions[3].center, size=design_regions[3].size, material=design_regions[3].design_parameters),
    #mp.Block(center=mp.Vector3(y=(block_height + sum(design_region_height))/2), size=mp.Vector3(Sx, block_height), material=SiO2)
    ]
...
kpoint = mp.Vector3(0, 0, 0) if args.p else None
...
# Focus point
focal_point = 4
far_x = [mp.Vector3(-3, focal_point, 0), mp.Vector3(0, focal_point, 0), mp.Vector3(3, focal_point, 0)]
...
def J1(FF):
    return min((np.abs(FF[0, 0, 2]) ** 2), (np.abs(FF[1, 1, 2]) ** 2), (np.abs(FF[2, 2, 2]) ** 2))
...
# Initial guess
# seed = np.random.randint(1, 1500) # make sure starting conditions are random, but always the same. Change seed to change starting conditions
# np.random.seed(seed)
...
for sample_nr in [args.it]:
	...
	x = np.random.rand(n) if args.r else np.ones(n) * 0.5
	#x = np.load("4l3fp_2/sample_4/x.npy")	
	if symmetry:
		for i in range(num_layers):
	...
	sendNotification("Opitimization " + str(sample_nr) + " started at " + start.strftime("%d/%m/%Y %H:%M:%S"))
	for iters in range(num_betas):
		...
		print("Current iteration: {}".format(cur_iter[0]) + "; " + str(int(100 * cur_iter[0] / totalIterations)) +
			  "% completed ; eta at " + str(start + estimatedSimulationTime))
		#sendNotification("Sample " + str(sample_nr) + ":*\n" + str(int(100 * (iters + 1) / num_betas)) + " % completed\n ETA at " + (start + estimatedSimulationTime).strftime("%d/%m/%Y %H:%M:%S"))
...
#, x[550:1099], x[1100:1649], x[1650:2199]])
	...
	sendNotification("Objective_value = " + str(f0))

	intensities = [np.abs(opt.get_objective_arguments()[0][0, 0, 2] ** 2), np.abs(opt.get_objective_arguments()[0][1, 1, 2] ** 2), np.abs(opt.get_objective_arguments()[0][2, 2, 2] ** 2)]
	...
	plt.savefig("./" + scriptName + "/" + scriptName_i + "/objective.png")
